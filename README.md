...
- run discovery to create data availability state file: `uv run utils/discovery.py --direction ASCENDING --track 22 --frame 157`

- run build_triplets to create RSLC pair state file for all possible 12, 24, and 36 day pairs: `uv run utils/build_triplets.py --direction ASCENDING --track 22 --frame 157`

- run the orchestrator which submits RSLC batches to snakemake, I recommend running in the background `nohup uv run nisar_orchestrator.py --jobs 4 --local-cores 4 --track 22 --frame 157 --direction ASCENDING --product-path /geod/ts-insar/NISAR_temp > NISAR_A_T22_F157.log 2>&1 &` (`--product-path` can also be `.` if you would like to save products into the local folder).
